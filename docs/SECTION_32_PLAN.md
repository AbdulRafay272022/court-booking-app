...
| `section-32-part-7` | `4713f85` | OCR improvements. **CONTINUED AND FINISHED + FULLY VERIFIED (2026-09-24). Pushed to origin. Not merged.** The owner reviewed the WIP diff (`fea54e5`) and said continue-and-finish. The WIP was ~90% complete and internally consistent: new payment fields (payer name, bank/wallet, receiver-tail, `ocr_flags`, and stored `name/time/receiver` verdicts), migration `a3f9c6e2d174` (7 nullable columns, additive), a strict no-guess vision prompt + structured-output schema across all three providers, the verdict engine + `build_payment_checks` (the five plain-language checks returned as ready-made strings), tightened auto-approve gating (name match + time within timer + receiver match-or-not_configured), and the owner approval-card UI on both platforms. The ONE real breakage was the web approvals page (`apps/web/app/dashboard/owner/approvals/page.tsx`) referencing a `<CheckRow>` it never defined -- fixed by adding it + a `CHECK_COLORS` map, mirroring mobile. **Verified this session:** real backend suite **515/515** on an isolated scratch Postgres (port 5544, never the shared dev DB; `AI_PROVIDER=claude AI_VISION_PROVIDER=claude`); migration tested upgrade->downgrade->upgrade on that scratch DB (7 columns added, dropped, re-added; `alembic check` clean, no drift); both apps typecheck clean (`next build` green for web, `tsc --noEmit` clean for mobile + packages); and **live-verified on BOTH platforms** (Playwright web production build at 1200px + Expo web at 390px, signed in as a real owner against the real local backend) that the card renders all five checks with correctly colored dots (mismatch=red, match=green, not_configured=grey). **FIXTURES + ACCURACY DONE (2026-09-24):** added synthetic JazzCash/Easypaisa fixtures (clean + messy + failed + not-a-receipt) in `court-booking-backend/tests/fixtures/payments/`, a generator, and `scripts/ocr_accuracy_report.py` (a real-vision-model script kept OUT of the pytest suite since it makes billed calls). Ran it against the real Gemini model: **32/32 (100%) on the synthetic images** (amount/reference/payer/bank/receiver/date/flags all correct). This is an OPTIMISTIC UPPER BOUND on rendered images -- real phone-screen photos are harder, so an honest real-world number still needs a handful of real (anonymized) pilot screenshots run through the same script (owner to supply when convenient). **Real bug this accuracy run caught + fixed:** the vision model returns the payment time exactly as printed ("24 Sep 2026, 07:12 PM"), NOT ISO 8601, so the old ISO-only `_parse_ocr_timestamp` returned None on every real screenshot -- silently making the time check always "not visible" and blocking auto-approve. The parser now accepts the human formats and reads the naive receipt time as PKT (treating it as UTC was 5h off); the prompt/schema wording was changed from "ISO 8601" to "as printed"; regression tests added. Backend suite now **522/522** on the isolated scratch DB (515 at finish + 7 new). No migration change from this follow-up. **Batch-merge note:** Parts 5, 10, and 7 each set `down_revision = de11b783f108`, so assembling the batch produces 3 Alembic heads that must be linearized (re-chain, or a merge migration) -- a one-time batch step affecting all three equally, not a Part 7 defect. |
...
| 6 | Photos and reviews | **Built + verified on branch `section-32-part-6`, not merged.** Backend: migration `c7d1a9b4e2f0` (reviews.is_hidden, reviews.updated_at, courts.photos; additive, tested up/down/up, no drift); `ReviewService` + endpoints (create on completed booking only, one-per-booking, PATCH edit within 7 days, owner reply, admin hide/unhide audit-logged, `/reviews/mine`, public list excludes hidden, avg+count over visible only, player_first_name); venue POST(limit 8)+PUT-ordered-keys and court photos (new gallery, limit 5) POST+PUT; `photo_urls`/`photo_keys` + `review_count` exposed. Frontend (web + mobile): photo management in Venue Settings (upload w/ client-side ~1600px compression, delete, reorder, set-cover, per-count limits) for venue + each court; reviews list on the venue page (avg + count, first name only, owner reply, admin Hide/Unhide); "Rate your game" + 7-day edit on completed bookings in My Bookings; owner Reviews screen with reply. **Verified:** backend suite **491/491** on an isolated scratch Postgres; both apps typecheck clean (web `next build` green, mobile `tsc` clean); web live Playwright pass (venue+court photo upload w/ cover/count, rate-your-game submit, venue reviews list, owner reply, admin hide) and mobile Expo-web pass (venue reviews, My Bookings rate, owner reviews). **Scope decisions (flagged):** reviews are venue-level (existing model has no court_id), so rating/count show at venue level, not per-court; wizard photo upload lives on the post-setup/pending screen + Venue Settings (not mid-draft, which would risk the fragile multi-step submit); mobile photo picker can't be driven headlessly (expo-image-picker limitation) so the upload path is typecheck+code-verified, not Playwright-driven; deleting a photo drops the reference (S3 object left as a harmless orphan). **Batch note:** this migration also revises `de11b783f108` (like Parts 5/7/10) -> linearize heads at batch assembly. |

...
| 7 | OCR improvements (updated rules below) | **Built + FULLY VERIFIED on branch `section-32-part-7` (2026-09-24), not merged to `main`.** New payment fields (payer name, bank/wallet, receiver-tail, flags, and stored name/time/receiver verdicts), migration `a3f9c6e2d174` (7 nullable columns, additive), strict vision prompt + structured-output schema across all three providers, the verdict engine + `build_payment_checks` (five plain-language checks as ready-made strings), tightened auto-approve gating (name match + time within timer + receiver match/not_configured), and the owner approval-card UI on both platforms. Continued from the untested WIP: the only real breakage was the web approvals page using an undefined `<CheckRow>` -- added it + a color map, mirroring mobile. Verified: backend **515/515** on an isolated scratch Postgres (port 5544, `AI_PROVIDER=claude AI_VISION_PROVIDER=claude`); migration up->down->up + `alembic check` clean (no drift); both apps typecheck clean (`next build` green, mobile `tsc` clean); live-verified on web (Playwright, 1200px) and Expo web (390px) signed in as a real owner -- the card renders all five checks with correct colored dots. **Fixtures + accuracy DONE (2026-09-24):** added synthetic JazzCash/Easypaisa fixtures (clean + messy + failed + not-a-receipt) in `tests/fixtures/payments/`, a generator, and `scripts/ocr_accuracy_report.py` (a real-vision-model script kept out of the pytest suite). Ran it against the real Gemini model: **32/32 (100%) on synthetic images** (amount/reference/payer/bank/receiver/date/flags all correct) -- but this is an OPTIMISTIC UPPER BOUND on rendered images; real phone-screen photos are harder, so an honest real-world number still needs a handful of real (anonymized) pilot screenshots run through the same script. **Real bug this accuracy run caught + fixed:** the vision model returns the payment time exactly as printed ("24 Sep 2026, 07:12 PM"), NOT ISO, so the old ISO-only `_parse_ocr_timestamp` returned None on every real screenshot -- silently making the time check always "not visible" and blocking auto-approve. Parser now accepts the human formats and reads the naive receipt time as PKT (treating it as UTC was 5h off); prompt/schema wording fixed to say "as printed" not "ISO 8601"; regression tests added. Backend now **522/522**. **Batch note:** Parts 5/10/7 migrations all revise `de11b783f108`, so the batch merge will need Alembic-head linearization (affects all three, not a Part 7 defect). |

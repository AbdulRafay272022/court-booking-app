...
| 9 | QR check-in (below) | **Backend done and tested (2026-09-23): 484/484 backend tests pass (478 + 6).** Most of the backend already existed (`checkin_qr_token` per venue, `POST /bookings/{id}/checkin` owner-scan, `POST /bookings/{id}/checkin/self` player self-scan, the automatic no-show job) -- added a **manual owner "Mark no-show" endpoint** (`POST /bookings/{id}/no-show`, refuses with `TOO_EARLY_FOR_NO_SHOW` until the same `NO_SHOW_GRACE_MINUTES` window the automatic job already uses has passed, so the two never disagree) since the spec says "owner marks it" and no manual path existed, only the automatic job. **Real bug found and fixed**: the availability engine only considers LIVE_BOOKING_STATUSES, so a checked-in or no-show booking's grid cell silently reverted to "available" on the owner's Today screen -- the checked-in/no-show record simply vanished, defeating "record who and when". This has been live in production since the automatic no-show job was wired up (Section 32 FCM pass) -- every real no-show since then has been invisible on Today. Fixed by overlaying completed/no-show bookings back onto their covered cell in `owner_dashboard_service.today()`; `TodaySlotOut` gained `checked_in_at`/`checked_in_by`. Frontend built on both platforms: Today gained Check-in/Mark no-show buttons on booked slots and correct completed/no-show display; a "Check-in tools" hub with a venue-QR display screen (owner) and a player check-in screen (own booking QR to show staff + self-scan gated to the ~15-minute window via a new shared `selfCheckinWindow` helper, with a manual code fallback). Mobile gets real camera scanning (`expo-camera` + `react-native-qrcode-svg`, new deps, needs an EAS build to verify on a device -- standing limitation); web deliberately gets QR display + manual code entry only, no live camera scanning (a laptop front desk isn't the realistic device for it; flagged, not silently decided) -- see the frontend CLAUDE.md's Part 9 entry for the full design and live-verification writeup. **All on branch `section-32-part-9`, off `main` (not off `section-32-part-5`, per the owner's instruction to keep parts independent), not merged.** |

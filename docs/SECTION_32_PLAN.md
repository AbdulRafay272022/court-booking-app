...
| 4b | Player venue page redesign | **DEPLOYED 2026-09-23 as `a70463ef7dc3`; migration `de11b783f108` ran from the new image before the backend restarted** (1 existing venue defaulted to 90-day horizon; head `de11b783f108`; `alembic check` clean). Calendar-first design (per-sport tabs, each court's own always-visible month calendar, date-tap popup with week strip + day slots) verified live on production (`https://3.6.48.6.sslip.io/venues/maidan-court`): real calendar dots, popup open/close, real slot data, zero errors in 10 minutes of post-deploy logs. **The first deploy attempt was blocked by an unrelated CI bug** in `infra/scripts/check-migration-guard.sh` (a `grep -q`/`pipefail` interaction that misread a genuine approval line as absent) -- fixed separately (commit `a70463e`, script-only, no `Migration-Go` line needed) and verified with a real repro harness before the second, successful push. Mobile needs an EAS build to reach real phones. |
| 5 | Split payments + `payment_entries` ledger | **Backend done and rigorously tested (2026-09-23): 494/494 backend tests pass (478 + 16 in `test_payment_entries.py`, covering partial payments summing to the total, over-payment rejected, two owners recording concurrently -- row-locked, never both take effect -- a cancelled booking's payments still appearing in the ledger, and both an owner correcting their own venue's entry and being refused on someone else's). Migration `0e808ff86785` (new `payment_entries` table + `courts.advance_type`/`advance_value`/`advance_minimum`) tested upgrade AND downgrade on a scratch DB seeded with production-shaped data -- backfill correctly infers method from booking source, downgrade leaves `bookings.amount_paid`/`balance_due` untouched, full up-down-up cycle clean.** Frontend built on both platforms: owner "Record payment" action on Today, the Ledger rewritten as one row per payment with collected-today/week/month + outstanding + refund-pending summary tiles, a per-court advance-rule card in Venue Settings, and (added after the owner's follow-up) a **"Correct a payment" action on each ledger row** -- shows the original amount/date/booking, requires a typed reason, confirms before submitting, and calls the reversal endpoint. **The reversal endpoint moved from admin-only to `RequireOwner`** (a venue owner can correct their own venue's entries, an admin any) after discovering the owner-facing Ledger screen is route-gated away from admin accounts entirely, so an admin-only endpoint reachable only from that screen would have been unusable by anyone -- flagged and confirmed with the owner before changing it. Live-verified against the real local backend end to end: recorded a payment, watched the balance update, reversed it, confirmed the balance returned to exactly what it was before and the ledger showed both the original and the reversal clearly (not a gap). **All on branch `section-32-part-5`, not merged to `main`, nothing pushed.** Owner to review before merging or starting the next part. |

...
| 7 | OCR improvements (updated rules below) | **Built + FULLY VERIFIED on branch `section-32-part-7` (`2943088`), pushed to origin, not merged.** Real 515/515 backend tests on an isolated scratch DB, migration tested up/down/up (`alembic check` clean), both apps typecheck clean, live Playwright/Expo-web-verified on both platforms. One spec item still outstanding (real fixture images + honest OCR-accuracy report on messy screenshots -- needs owner-supplied real screenshots). See SESSION HANDOFF above. |

...
| 10 | Refunds, manual (below) | **Built and tested on branch `section-32-part-10` (2026-09-24), not merged to `main`, nothing pushed to the actual pilot deploy pipeline.** Reuses `payment_disputes` (migration `a3f7c9d2e185`, upgrade+downgrade+upgrade verified clean on an isolated scratch DB, `alembic check` clean) rather than a new table -- see the backend CLAUDE.md's Section 32 Part 10 entry for the full design, the cutoff-policy decision that was needed before this could be built, and the Part 5 (`payment_entries`) dependency note. 485 backend tests passing (478 on `main` + 7 new), both frontend apps typecheck clean. Pushed to `origin/section-32-part-10` as a branch for backup, same as the other in-flight parts. |

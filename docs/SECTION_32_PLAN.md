...
| 5 | Split payments + `payment_entries` ledger | **Backend done and rigorously tested (2026-09-23): 493/493 backend tests pass (478 + 15 new in `test_payment_entries.py`, covering partial payments summing to the total, over-payment rejected, two owners recording concurrently -- row-locked, never both take effect -- and a cancelled booking's payments still appearing in the ledger). Migration `0e808ff86785` (new `payment_entries` table + `courts.advance_type`/`advance_value`/`advance_minimum`) tested upgrade AND downgrade on a scratch DB seeded with production-shaped data (app-paid, walk-in-paid, and unpaid bookings) -- backfill correctly infers method from booking source, downgrade leaves `bookings.amount_paid`/`balance_due` untouched, full up-down-up cycle clean.** Frontend built on both platforms: owner "Record payment" action on Today (row-locked against overpay, same as the backend), the Ledger rewritten as one row per payment with collected-today/week/month + outstanding + refund-pending summary tiles, and a per-court advance-rule card in Venue Settings (fixed/percent/minimum, falling back to the pricing rule). Live-verified against the real local backend: recorded a payment, watched the balance update, confirmed the ledger showed both entries with correct running totals, changed the advance rule and confirmed it persisted. **All on branch `section-32-part-5`, not merged to `main`, nothing pushed.** Admin's "view and correct payments" (reversing entries) is built and tested on the backend (`POST /admin/payment-entries/{id}/reverse`) but has no dedicated admin UI yet -- deliberately deferred, reachable via `/docs` for now, flagged rather than silently decided. Owner to review before merging or starting the next part. |
